...
Using the full GPT-4.1 model for combat instead of the mini version.
"""
...
TEST_FILES = {
    ...
    "prompt": os.path.join(DUMMY_DIR, "combat_sim_prompt_with_data.txt")
}
...
def format_system_prompt(player_stats, npc_stats_list, monster_stats_list, location_info):
    ...
    try:
        ...
        # Apply the specific placeholder formats in our new template
        replacements = {
            "{player_name}": player_stats['name'],
            "{player_class}": player_stats['class'],
            "{player_level}": str(player_stats['level']),
            "{player_hp}": str(player_stats['hitPoints']),
            "{player_max_hp}": str(player_stats['maxHitPoints']),
            "{player_ac}": str(player_stats['armorClass']),
            "{player_str}": str(player_stats['abilities']['strength']),
            "{player_str_mod}": str((player_stats['abilities']['strength']-10)//2),
            "{player_dex}": str(player_stats['abilities']['dexterity']),
            "{player_dex_mod}": str((player_stats['abilities']['dexterity']-10)//2),
            "{player_con}": str(player_stats['abilities']['constitution']),
            "{player_con_mod}": str((player_stats['abilities']['constitution']-10)//2),
            "{player_int}": str(player_stats['abilities']['intelligence']),
            "{player_int_mod}": str((player_stats['abilities']['intelligence']-10)//2),
            "{player_wis}": str(player_stats['abilities']['wisdom']),
            "{player_wis_mod}": str((player_stats['abilities']['wisdom']-10)//2),
            "{player_cha}": str(player_stats['abilities']['charisma']),
            "{player_cha_mod}": str((player_stats['abilities']['charisma']-10)//2),
            "{player_skills}": skills_str,
            "{player_attacks}": attacks_str,
            "{player_features}": features_str,
            "{npc_section}": npc_section,
            "{monster_section}": monster_section,
            "{location_name}": location_info.get('name', 'Unknown Location'),
            "{location_description}": location_info.get('description', 'No description available')
        }
...
def run_combat_test():
    ...
    combat_prompt = format_system_prompt(
        player_stats,
        [npc_stats],
        [monster_stats],
        location_info
    ...
    # Debug - Print the first 500 characters of the formatted prompt
    print(f"\n{YELLOW}DEBUG - First 500 chars of formatted prompt:{RESET_COLOR}")
    print(f"{combat_prompt[:500]}...")

    # Debug - Check for placeholder strings that weren't replaced
    remaining_placeholders = []
    for placeholder in ["{player_stats", "{npc_section", "{monster_section", "{location_info"]:
        if placeholder in combat_prompt:
            remaining_placeholders.append(placeholder)

    if remaining_placeholders:
        print(f"\n{SOFT_REDDISH_ORANGE}WARNING - Unreplaced placeholders found:{RESET_COLOR}")
        for placeholder in remaining_placeholders:
            print(f"- {placeholder}")
    else:
        print(f"\n{SOLID_GREEN}All major placeholders replaced successfully{RESET_COLOR}")

    # Debug - Check for key monster and NPC names
    expected_names = [
        player_stats['name'],
        npc_stats['name'],
        monster_stats['name']
    ...
    for name in expected_names:
        if name not in combat_prompt:
            missing_names.append(name)

    if missing_names:
        print(f"\n{SOFT_REDDISH_ORANGE}WARNING - Expected names missing from prompt:{RESET_COLOR}")
        for name in missing_names:
            print(f"- {name}")
    else:
        print(f"{SOLID_GREEN}All expected character names found in prompt{RESET_COLOR}")
